...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Entrena y prueba un agente Q-learning para Frozen Lake.")
    parser.add_argument('--episodes', type=int, default=100_000, help='Número de episodios de entrenamiento')
    ...
    parser.add_argument('--epsilon_min', type=float, default=0.03, help='Tasa de exploración mínima')
    parser.add_argument('--epsilon_decay', type=float, default=0.997, help='Decaimiento de epsilon')
    parser.add_argument('--test_episodes', type=int, default=10, help='Número de episodios de prueba')
    parser.add_argument('--save_path', type=str, default='q_table.npy', help='Ruta para guardar/cargar la Q-table')
...
def render_rgb_array(env, state, step, episode):
    # Renderiza el entorno como una imagen RGB y la muestra con Matplotlib
    img = env.render()
    plt.imshow(img)
    plt.title(f"Episodio {episode + 1}, Paso {step + 1}")
    plt.axis('off')
    plt.pause(0.05)  # Pausa breve para visualización
...
def train_agent(args):
    ...
    for episode in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if done:
                if reward == 1:  # Incrementa éxitos si se llega al objetivo
                    successes += 1
                break
        ...
        if (episode + 1) % 10000 == 0:
            ...
            print(f"Episodios {episode + 1 - 9999}-{episode + 1}: Tasa de éxito: {success_rate:.2f}%, Recompensa promedio: {avg_reward:.4f}")
        ...
        for step in range(args.max_steps):
            try:
                if args.render_mode == 'human':
                    ...
                    render_rgb_array(env, state, step, ep)  # Renderiza con Matplotlib
            ...
            if done:
                if args.render_mode == 'human':
                    ...
                    render_rgb_array(env, state, step, ep)
                print(f"Episodio {ep + 1} terminado. Recompensa total: {total_reward}")
                if reward == 1:
                    print("¡Éxito: Llegaste al objetivo!")
                else:
