...
def parse_arguments():
    ...
    parser.add_argument('--episodes', type=int, default=2_000_000, help='Número de episodios de entrenamiento')
    ...
    parser.add_argument('--epsilon_decay', type=float, default=0.9998, help='Decaimiento de epsilon en etapas tardías')
    parser.add_argument('--step_penalty', type=float, default=-0.02, help='Penalización por paso para minimizar trayectorias')
    parser.add_argument('--goal_proximity_reward', type=float, default=0.2, help='Recompensa por alcanzar estados cercanos al objetivo')
    parser.add_argument('--test_episodes', type=int, default=100, help='Número de episodios de prueba')
    parser.add_argument('--save_path', type=str, default='q_table_opt_steps_v6.npy', help='Ruta para guardar/cargar la Q-table')
...
def train_agent(args):
    ...
    max_steps = 50 if not args.is_slippery else args.max_steps
    total_episodes = 200_000 if not args.is_slippery else args.episodes
    ...
    for episode in range(total_episodes):
        ...
        if episode < 300_000:
            epsilon = max(args.epsilon_min, epsilon * 0.991)
        else:
        ...
        if (episode + 1) % args.print_interval == 0:
            ...
            success_count = min(len(steps_in_success), 1000)
...
def test_agent(q_table, args):
    ...
    for ep in range(args.test_episodes):
        ...
        for _ in range(50 if not args.is_slippery else args.max_steps):
