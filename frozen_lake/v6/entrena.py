...
def parse_arguments():
    ...
    parser.add_argument('--episodes', type=int, default=400_000, help='Número de episodios de entrenamiento')
    ...
    parser.add_argument('--epsilon_min', type=float, default=0.005, help='Tasa de exploración mínima')
    ...
    parser.add_argument('--save_path', type=str, default='q_table_opt_steps_v2.npy', help='Ruta para guardar/cargar la Q-table')
...
def train_agent(args):
    ...
    for episode in range(args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if done:
                if reward > 0:  # Incrementa éxitos si se llega al objetivo
                    ...
                    if 6 <= steps <= 10:  # Considera 6-10 pasos como "casi óptimo"
                        optimal_steps += 1
                break
        ...
        if episode < 100_000:
            epsilon = max(args.epsilon_min, epsilon * 0.995)  # Decaimiento más rápido al inicio
        else:
            epsilon = max(args.epsilon_min, epsilon * args.epsilon_decay)  # Decaimiento más lento después
        ...
        if (episode + 1) % 10000 == 0:
            ...
            print(f"Episodios {episode + 1 - 9999}-{episode + 1}: Tasa de éxito: {success_rate:.2f}%, "
                  ...
                  f"Éxitos en 6-10 pasos: {optimal_rate:.2f}%")
